...
export default function TreeVisualization({
  initialTree = [50, 30, 70, 20, 40, 60, 80],
}: TreeVisualizationProps) {
  ...
  useEffect(() => {
    ...
    const drawTree = (
      ...
      if (node.left) {
        ctx.beginPath();
        ctx.moveTo(
          x - nodeRadius * Math.cos(Math.PI / 4),
          y + nodeRadius * Math.sin(Math.PI / 4)
        );
        ctx.lineTo(
          leftX + nodeRadius * Math.cos(Math.PI / 4),
          nextY - nodeRadius * Math.sin(Math.PI / 4)
        );
        ctx.strokeStyle = "hsl(var(--border))";
        ctx.lineWidth = 2;
        ctx.stroke();
      ...
      if (node.right) {
        ...
        ctx.lineTo(
          rightX - nodeRadius * Math.cos(Math.PI / 4),
          nextY - nodeRadius * Math.sin(Math.PI / 4)
        );
        ctx.strokeStyle = "hsl(var(--border))";
        ctx.lineWidth = 2;
    ...
    <div className="space-y-6 p-4 bg-card rounded-lg w-[70%]">
      <div className="space-y-2">
        ...
        <p className="text-muted-foreground">
          Un árbol binario de búsqueda es una estructura de datos jerárquica
          donde cada nodo tiene como máximo dos hijos. Para cada nodo, todos los
          elementos en el subárbol izquierdo son menores que el nodo, y todos
          los elementos en el subárbol derecho son mayores.
        </p>
      </div>
        ...
        <Tabs defaultValue="operations" className="w-full">
                ...
                <div className="flex flex-wrap gap-2">
                  {traversalResult.length > 0 ? (
                    ...
                    <span className="text-muted-foreground italic">
                      Selecciona un tipo de recorrido
                    </span>
                  )}
                </div>
              </div>
          ...
          <TabsContent value="info" className="space-y-4">
            <div className="grid grid-cols-2 gap-2 text-sm">
              <div className="p-2 bg-muted rounded-md">
                <span className="font-semibold">Altura del árbol:</span>{" "}
                {treeHeight}
              </div>
              <div className="p-2 bg-muted rounded-md">
                <span className="font-semibold">Número de nodos:</span>{" "}
            ...
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-2 gap-2 text-sm">
                <div className="p-2 bg-muted rounded-md">
                  <span className="font-semibold">Búsqueda:</span> O(log n)
                  promedio, O(n) peor caso
                </div>
                <div className="p-2 bg-muted rounded-md">
                  <span className="font-semibold">Inserción:</span> O(log n)
                  promedio, O(n) peor caso
                </div>
                <div className="p-2 bg-muted rounded-md">
                  <span className="font-semibold">Eliminación:</span> O(log n)
                  promedio, O(n) peor caso
                </div>
                <div className="p-2 bg-muted rounded-md">
            ...
            <div className="bg-muted p-4 rounded-md">
              ...
              <ul className="list-disc list-inside space-y-1 text-sm">
                ...
                <li>
                  Implementación de conjuntos y mapas en lenguajes de
                  programación
                </li>
              </ul>
            </div>
            ...
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm">
                <div className="p-2 bg-muted rounded-md">
                  <span className="font-semibold">Árbol binario:</span> Cada
                  nodo tiene máximo dos hijos
                </div>
                <div className="p-2 bg-muted rounded-md">
                  <span className="font-semibold">Árbol binario completo:</span>{" "}
                  Todos los niveles están llenos, excepto posiblemente el último
                </div>
                <div className="p-2 bg-muted rounded-md">
                  <span className="font-semibold">Árbol AVL:</span> BST
                  autobalanceado donde la diferencia de altura entre subárboles
                  es ≤ 1
                </div>
                <div className="p-2 bg-muted rounded-md">
                  <span className="font-semibold">Árbol Rojo-Negro:</span> BST
                  autobalanceado con propiedades de coloración
                </div>
              </div>
            </div>
          </TabsContent>
